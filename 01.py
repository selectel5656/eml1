...
Работать все на node.js, все аккаунты должны делать соединения с сервером через прокси сокс5, прокси выбирается рандомно. У прокси ставится метка что он уже взят, метки сбрасываются на все прокси когда они закончатся. чтобы брать их еще раз.
...
{{$id_attach_1}} - идентификатор pdf-файла, полученный при загрузке; его следует передавать в `att_ids` при отправке.
